...
Password cracking is a useful skill for any penetration tester, red teamer, or even those on the defensive side of information security. During an assessment we will often retrieve a password hash that we must attempt to crack offline to proceed further towards our goal. 
...
It's not always possible to identify the algorithm based on the obtained hash. The plaintext might undergo multiple encryption rounds and salting transformations, making it difficult to recover. The **hashid** tool uses regular expressions to make a **best-effort guess**. Oftentimes **hashid** will provide many possibilities for a given hash, leaving us with a certain amount of guesswork in order to identify the hash.

**Hashcat** provides an excellent reference, that maps hash modes to example hashes. This reference is very handy when determining the type of hash, and the associated **hash mode** required for hashcat cracking:
...
Hashcat supports five different attack modes. Each mode have different applications depending on the type of hash and the complexity of the password.
...
A mask can be created using static characters, ranges of characters ([a-z], [A-Z0-9]), or placeholders.

The following list shows important placeholders:
...
|`?a` 		   |	`printable characters (?l?u?d?s)`       |
|`?b` 		   |	`0x00 - 0xff`                               |
...
A benchmark test measure your system's raw password cracking speed for different algorithms. This is done by running optimized cracking sessions, showing results in hashes per second (H/s). This information is useful when you want to optimize cracking speed through certain settings. 
...
- Optimized Kernels
    - The **-O** flag enables optimized kernels. This limits the password length, usually to 31 characters. Any password exceeding this limit will be skipped.
	
- Workload
    - The **-w** flag enables a specific workload profile (1 - 4). In other words, how aggressively Hashcat uses your hardware. The default profile is 2. If you want to use your computer while cracking, set the profile to 1. If you plan on only running Hashcat, the workload profile can be set to 3. If you have a dedicated cracking rig, set the profile to 4.
